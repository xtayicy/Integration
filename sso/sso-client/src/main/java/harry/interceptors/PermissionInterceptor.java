...
public class PermissionInterceptor extends HandlerInterceptorAdapter{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		Set<String> applicationPermissionSet = ApplicationUtil.getApplicationPermission(request);
	...
	public SessionPermission invokePermissionInSession(HttpServletRequest request) {
		...
		List<RpcPermission> dbList = authenticationRpcService.findPermissionList(user.getToken(),null);
		...
		Set<String> noPermissionSet = new HashSet<String>(ApplicationUtil.getApplicationPermission(request));
		noPermissionSet.removeAll(operateSet);
		sessionPermission.setNoPermissions(StringUtil.join(noPermissionSet.toArray(), ","));

		// 保存登录用户权限列表
		sessionPermission.setPermissionSet(operateSet);
		ApplicationUtil.setSessionPermission(request, sessionPermission);
